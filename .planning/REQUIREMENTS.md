...
- [ ] **SYNTH-01**: Operator can run `npx tsx scripts/synthesize-delta.ts --organic <stamp> --target-days N --seed NN --milestone <name>` to produce a fused organic+synthetic fixture under `tests/fixtures/primed/<milestone>-<N>days/`.
- [ ] **SYNTH-02**: Synthetic pensieve entries are generated by a Haiku call per synthetic day that uses randomly-sampled organic telegram entries as few-shot examples; output entries carry realistic UTC+1/+2 timestamps and `source='telegram'`.
- [ ] **SYNTH-03**: Synthetic episodic summaries are produced by running the real `runConsolidate(date)` from `src/episodic/consolidate.ts` against each synthetic day — the engine under test produces the fixture's summaries.
- [ ] **SYNTH-04**: Synthetic decisions carry realistic `resolve_by` spread. If the consuming milestone requires resolutions, synthesize resolution replies and run real `handleResolution`.
- [ ] **SYNTH-05**: Synthetic contradiction pairs are deterministic: N pre-written adversarial pairs inserted with `status='DETECTED'`.
- [ ] **SYNTH-06**: Synthetic `wellbeing_snapshots` rows are generated with realistic 1–5 distributions (for M009 forward use; schema may be added by M009 itself — v2.3 codes against whatever table exists).
- [ ] **SYNTH-07**: Re-running `synthesize-delta.ts` with the same `--seed` against the same organic base produces byte-identical non-LLM output. Anthropic outputs are captured VCR-style on first run and replayed on subsequent runs.
...
- [ ] **FRESH-02**: A `--no-refresh` flag on `synthesize-delta.ts` / `regenerate-primed.ts` skips the auto-fetch and uses whatever `LATEST` points to (for offline / sandbox / air-gapped workflows only).
