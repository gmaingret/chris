...
- [ ] **RIT-10**: Atomic `UPDATE rituals SET last_run_at=now(), next_run_at=$NEW WHERE id=$ID AND last_run_at IS NULL OR last_run_at < $LAST_OBSERVED RETURNING *` idempotency (write-before-send, mirrors M007 D-28). Prevents double-fire under cron-tick race.
- [ ] **RIT-11**: Second cron tick at 21:00 Europe/Paris registered in `src/index.ts` (peer to existing 10:00 sweep + 23:00 episodic). Morning 10:00 catches accountability + reflective + 09:00 wellbeing; evening 21:00 catches voice note + Sunday weekly review.
- [ ] **RIT-12**: `src/config.ts` exports `ritualSweepCron` env var with safe default `0 21 * * *`; `cron.validate` at config load short-circuits on invalid expression. `/health` endpoint reports `ritual_cron_registered`.
