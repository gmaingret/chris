...
- [ ] **Phase 32: Substrate Hardening (post-incident response 2026-05-05/07 + Phase 30 carry-forward)** — Backlog captured from two sources. **Incident response 2026-05-05/07:** (1) **proactive prompt directive-following** — Sonnet ignored "BEFORE asking ANY question, scan Recent Conversation" directive and emitted a verbatim duplicate opener asking if Greg's in Batumi while Recent Conversation showed him already there discussing it; needs structural fix (Stage-2 Haiku verifier OR prompt restructure with few-shot BAD/GOOD pairs OR anti-repetition guard checking last 7d ASSISTANT openers). (2) **proactive context-builder edge case** — when 48h Recent Conversation window contains zero USER messages, model meta-comments about its own "Recent Conversation" + "Pensieve" sections (commit 28432bd patches the symptom via `detectAbstention()` guard but root cause is the prompt structure inviting meta-commentary); add `skip-when-no-USER-messages-in-48h` guard in `runReflectiveChannel`. (3) **migration journal monotonic-`when` CI guardrail** — assert journal entries' `when` values are monotonically increasing in CI (would have caught the year-stale `when: 1746576000000` typo on 0011 from commit `ac8fd98` that gated drizzle behind a stale `when` until commit be22af0 fixed it on 2026-05-06). (4) **boot-time `__drizzle_migrations` drift warning** — in `src/db/migrate.ts` after `migrate()` completes, log warn if `count(*) FROM drizzle.__drizzle_migrations != journal.entries.length`; would have surfaced the 2026-05-04 row-loss + re-application incident weeks earlier. (5) **investigate upstream cause of `__drizzle_migrations` row loss** (forensic trail cold; check Proxmox snapshot/restore audit if any). **Phase 30 carry-forward (synth pipeline gaps surfaced during 30-01):** (6) **`synthesize-episodic.ts:288` organic fusion** — currently skips loading organic `episodic_summaries.jsonl` into the throwaway DB before re-consolidating; result: m009-21days fixture has 4 synth-only episodic summaries instead of organic-fused 20+. After this lands, HARN-04 invariant in `src/__tests__/fixtures/primed-sanity.test.ts` reverts from `MIN_EPISODIC_SUMMARIES = 4` (relaxed) to the original `≥21`. (7) **`synthesize-delta.ts` wellbeing-per-fused-day generation** — currently emits 1 wellbeing_snapshot per synth day only (4 rows for 4 synth days). HARN-06's `≥14` invariant needs synth to also generate wellbeing rows for organic days that lack them, OR fuse organic wellbeing into the dump. After this lands, the 5th invariant in `primed-sanity.test.ts` reverts from `≥4` to `≥14`. (8) **`src/rituals/journal.ts:357` lastIdx formula** — bag-empty branch returns `undefined` so the consecutive-duplicate guard cannot defend at cycle boundaries; ~17% chance of consecutive-duplicate openers per cycle boundary. Surfaced by Phase 30 verification. (9) **weekly_review Stage-1 + observation-length regression on adversarial input** — TEST-31 first live run on 2026-05-07 against real Sonnet: 2/3 iterations triggered the templated fallback (`"What stood out to you about this week?"`) instead of returning a substantive observation. Two distinct failure modes hit Sonnet under the adversarial-week fixture: (a) Stage-1 violation (`question` field contained multi-`?` OR multi-interrogative-leading-word per EN/FR/RU regex) and (b) observation length exceeded the Zod `.max(800)` constraint. Retries exhausted → fallback. Real-world Greg-week input contains the same emotional-content patterns the fixture is designed to bait, so May 10 weekly_review fire risks producing the templated fallback in production. Phase 29's prompt enforcement is the SUT; fix lives in `src/rituals/weekly-review-prompt.ts` (tighten single-question directive + add explicit observation length cap with worked example). After this lands, TEST-31 should pass 3/3 (`expect(fallbacks).toBe(0)`). Does NOT belong in M009 v2.4 — touches DB substrate (M001) + proactive engine (M004/M008) + test-data substrate (v2.3 Phase 24) + ritual prompt rotation (M009 minor) + weekly_review prompt design (M009 — but the M009 milestone ships with TEST-31 documented-failing per Greg's call 2026-05-07), not the M009 milestone-shipping path. **Belongs in v2.5 (new milestone).** Source artifacts: `.planning/debug/m009-rituals-no-fire.md`, commits `be22af0` + `28432bd`, `.planning/phases/30-test-infrastructure-harn-03-refresh/30-01-SUMMARY.md` + `30-VERIFICATION.md` + `deferred-items.md`.
...
- [x] 30-01-PLAN.md — HARN fixture refresh substrate: --reseed-vcr flag wiring + m009-21days regeneration + MANIFEST fail-fast invariants (Sunday=ISO weekday 7) + primed-sanity threshold flip 7→21 + 5th wellbeing_snapshots≥14 invariant + VCR cost model docs (HARN-04, HARN-05, HARN-06)
- [x] 30-02-PLAN.md — Synthetic fixture test (TEST-23..30 ATOMIC): src/rituals/__tests__/synthetic-fixture.test.ts NEW with 14-day vi.setSystemTime walk + cumulative afterAll mockAnthropicCreate.not.toHaveBeenCalled (Pitfall 6 regression) + 6 it() blocks across 8 TEST IDs + simulateCallbackQuery helper extraction + chat-id BigInt(99921) registry slot + ritual.next_run_at reset (TEST-23, TEST-24, TEST-25, TEST-26, TEST-27, TEST-28, TEST-29, TEST-30)
- [x] 30-03-PLAN.md — Cron registration regression EXTENSION (HARD CO-LOC #4): append 5th it() block to existing src/rituals/__tests__/cron-registration.test.ts asserting registerCrons invoked from src/index.ts main() with all 4 M009 handlers including ritualConfirmationSweep via static fs.readFile analysis (TEST-32)
- [x] 30-04-PLAN.md — Live anti-flattery gate flip (HARD CO-LOC #6): tighten src/rituals/__tests__/live-weekly-review.test.ts skipIf to dual-gate RUN_LIVE_TESTS+ANTHROPIC_API_KEY per D-30-03 cost discipline + reword Phase-30 marker comments + extend TESTING.md Live Tests table with M009 row + manual invocation pattern (TEST-31)
...
| 30. Test Infrastructure + HARN-03 Refresh | v2.4 | 4/4 | Complete   | 2026-05-07 |
