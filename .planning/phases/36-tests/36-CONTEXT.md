...
- **D-19: Three-cycle assertion structure** (mirrors Phase 34 D-36 — corrected 2026-05-13 after pattern-mapper review):
  ...
  - **Cycle 3:** mutate the fixture's pensieve_entries (e.g., insert one new FACT-tagged entry in the jurisdictional domain via `db.insert(pensieveEntries).values({...})`); re-run; assert `mockAnthropicParse.toHaveBeenCalledTimes(8)` (4 NEW calls — **NOT 5**). **Why 8, not 5:** Phase 34 D-14 uses a single SHARED substrate across all 4 generators (`loadProfileSubstrate` is called ONCE per fire). The substrate-hash signature (Phase 34 D-15) is computed over the full substrate (`pensieveIds.sort() + episodicDates.sort() + decisionIds.sort()` + `prevStateMeta.schema_version`). Any Pensieve mutation invalidates ALL 4 dimensions' hashes simultaneously. The "per-dimension comparison is independent" guarantee from Phase 34 D-17 still holds (each generator checks its own column), but the inputs to all 4 hashes change together. Assert all 4 outcomes are `'profile_updated'`; assert `profile_history` has 8 rows now (4 from Cycle 1 + 4 from Cycle 3).
  - **Implication for production:** any week where Greg adds a single FACT-tagged Pensieve entry triggers all 4 generators on the next Sunday 22:00 fire. This is the explicit v1 trade-off from Phase 34 D-14 (simplicity > selective regeneration cost). Per-dimension substrate views are a v2.5.1 candidate if API spend becomes a concern.
