...
3. Inserts a `ritual_responses` row linking the Pensieve entry id back to the ritual + fire timestamp (so longitudinal queries can follow the prompt → response → Pensieve chain). **`ritual_responses.prompt_text` is sourced from `ritual_pending_responses.prompt_text`** — see column addition in migration 0007 below. The pending row's `prompt_text` is set by `fireVoiceNote` at fire time (PROMPTS[promptIdx]); the consumed row's `prompt_text` is read into the `ritual_responses` insert in `recordRitualVoiceResponse`. **No empty-string placeholder, no NOT NULL violation risk.**
...
- **Column addition in migration 0007 (added 2026-04-27 post-checker B4):** `ALTER TABLE ritual_pending_responses ADD COLUMN IF NOT EXISTS prompt_text text NOT NULL DEFAULT '';` followed by `ALTER TABLE ritual_pending_responses ALTER COLUMN prompt_text DROP DEFAULT;`. `src/db/schema.ts` ritualPendingResponses gets a corresponding `promptText: text('prompt_text').notNull()` declaration. The DEFAULT-then-DROP-DEFAULT pattern handles the (currently zero-row) table at deploy time without backfill ambiguity. **Why this column exists:** PP#5's `recordRitualVoiceResponse` reads `prompt_text` back from the consumed pending row to populate `ritual_responses.prompt_text` — the longitudinal prompt → response → Pensieve chain depends on it being non-empty.
