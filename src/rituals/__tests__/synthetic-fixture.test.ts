...
skipIfAbsent('M009 synthetic fixture (14 days; TEST-23..30)', () => {
  ...
  // 2026-05-12: bumped timeout 60s→180s. 14 simulated journal replies
  // serialize through Pensieve store (~3s each on CPU fp32 with bge-m3),
  // plus the first-call model-load can add 30s. 60s was too tight under
  // the post-`.env`-fix world where the model actually attempts to load
  // properly.
  it('TEST-25: 14 days of journal replies persist as RITUAL_RESPONSE; PP#5 short-circuit (Pitfall 6 cumulative — see afterAll)', { timeout: 180_000 }, async () => {
    await parkOtherRituals();
