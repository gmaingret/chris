 ...
 *      the first already advanced `last_run_at` past `$observed`.
...
export async function tryFireRitualAtomic(
  ...
  //   lastObserved !== null   →  or(isNull(lastRunAt), lt(lastRunAt, lastObserved))
  //
  // The null-observation case is the load-bearing branch for the concurrency
  // contract. If we used a row-state-INDEPENDENT predicate (e.g. `sql\`true\``)
  // for the null case, two concurrent UPDATEs would both pass the predicate
  // even after the first commits — postgres row-level locking would
  // serialize them, but the second's WHERE re-evaluation would still
  // succeed because the predicate doesn't depend on the row's current
  // last_run_at. By restricting the null case to `isNull(lastRunAt)`, the
  // second invocation's WHERE re-evaluation FAILS (the row's lastRunAt is
  // now non-null after the first commit), so the second invocation gets
  // zero rows back. This is the SQL-level proof of RIT-10's exactly-once
  // guarantee under concurrency.
  //
  // The non-null case retains the OR(isNull, lt) shape so a peer that
  ...
  const lastRunAtPredicate = lastObserved
    ? or(isNull(rituals.lastRunAt), lt(rituals.lastRunAt, lastObserved))
